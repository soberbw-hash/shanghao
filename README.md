...
- 通过原声与处理后信号的协作，减轻回声消除和强降噪同时开启时吞掉本人声音的问题。
...
- 单条录音可收藏或删除，也能一键清理不足 10 秒的短录音。
- 存储上限默认 10 GB，超过后从最旧录音开始整理，并至少保留一条录音。
- 录音完成后可直接打开保存文件夹，路径以适合阅读的方式显示。

### 🧠 9. AI 语音记忆基础

2.8.0 已经包含 AI 语音记忆的模型管理与资源调度基础，但**尚未把完整转录、总结、章节、问答和精彩片段推理链作为可用成品对外承诺**。
...
设置中的“自动转录”和“自动整理”会在缺少模型时明确提示依赖。真正的录音转写和内容理解能力完成并经过本地验证前，本 README 不把规划项写成已支持功能。
...
- 轻量语音悬浮窗显示成员状态；停留后可沿垂直方向吸附移动并恢复默认位置。
- 全局静音与按键说话快捷键。
...
当前 AI 语音记忆只完成本地模型管理与任务调度基础，模型首次需要用户手动下载。设计边界是本地处理，原始录音默认不上传；完整推理功能尚未在 README 中宣称可用。
...
当前正式版本为 **2.8.0**。这是 2.6.0 之后第一次对外发布，合并了本地 2.6.1、2.7.0 与 2.8.0 三个开发阶段的全部改进。
...
版本细节见 [CHANGELOG.md](./CHANGELOG.md)，普通用户说明见 [v2.8.0 更新公告](./docs/release-notes/v2.8.0.md)。
