...
> [!TIP]
> `main` 当前为 v3.0.4，新增多模型录音对比、可恢复的后台转录、AAC 快捷语音库和统一音量设置，并继续保留 3.0.3 的账号、AI Runtime、1440p 屏幕分享与房间反馈能力。完整变化见 [v3.0.4 更新公告](./docs/release-notes/v3.0.4.md)。
...
- Paraformer 中文套件、Qwen3-ASR 0.6B/1.7B、Fun-ASR-Nano、GLM-ASR-Nano、FireRedASR2-AED、MOSS-Transcribe-Diarize、Dolphin-CN-Dialect 与 Cohere Transcribe 九套主转录方案。
- Qwen3-ForcedAligner 作为共享精确对齐组件，Qwen3.5-4B 独立负责本地整理，不重复下载公共权重。
- GPU 模型复用 ShangHao 独立 CUDA Runtime，自检 PyTorch、CUDA、GPU、BF16/FP16、显存和 DLL，不依赖系统 Python。
- 首次由用户主动下载，不把数 GB 权重塞进安装包，也不首次启动偷偷下载。
- 下载、暂停、继续、删除、进度显示、断点续传，以及下载、授权、完整性和 Runtime 四类错误状态。
- Cohere 门控模型支持 Windows 加密保存 Hugging Face 只读 Token，授权只发送给官方 `huggingface.co`。
...
- 支持用户名、邮箱和密码注册，提供十套本地默认头像，也可以上传自己的图片。
- 支持邮箱登录、Supabase 托管的用户名登录、自动登录、Session 刷新、退出登录、昵称和头像更新。
- userId 来自 Supabase Auth UUID；房间服务器验签 Access Token 后取得真实身份，不相信客户端自报 userId。
- Session 由 Windows 安全存储加密；密码与 Refresh Token 不进入 Renderer、普通配置或日志。
- 开发测试可保留现有 `ws://` 房间服务器，但密码与 Session 刷新仍只访问 Supabase HTTPS；正式部署默认要求 WSS/HTTPS。
...
- 完整名单见 [支持的游戏](./docs/supported-activities.md)。

### 🎵 12. 音乐状态

当前识别 Spotify、Apple Music、网易云音乐和 QQ 音乐，并显示应用及可获得的媒体会话信息。
...
| 转录模型、录音与转录结果     | 用户电脑本地、独立于安装目录                                        |
| 可选的整理与房间云端问答     | 经已加入的房间连接发送到自托管 Relay，再由 Relay 调用配置的 AI 服务 |
| 游戏和音乐状态               | 本机识别后只同步必要的状态名称/标识                                 |
...
- 不把云端 AI 密钥写入客户端、设置文件、日志、安装包或 GitHub。
...
- 把 `RELAY_ACCESS_TOKEN`、`TURN_SHARED_SECRET` 和 `DEEPSEEK_API_KEY` 当成密码保管。
...
不会读取游戏内存。检测只使用必要的 Windows 进程、路径和窗口证据，也不会注入或模拟输入。
...
当前仓库版本为 **3.0.4**，此前正式版本为 3.0.3。

近期几个重要阶段：

- **3.0.4**：新增多模型录音对比和断点恢复、AAC 快捷语音库、搜索/游戏/主播/未分类筛选、语音包导出、统一音量和 20GB 录音自动清理上限，并压缩账号、AI 与设置界面。详见 [v3.0.4 更新公告](./docs/release-notes/v3.0.4.md)。

- **3.0.3**：加入正式账号基础、九套 ASR 与统一 CUDA Runtime，修复门控模型授权、官方文件校验、字级转录显示和模型选择同步；同时升级 1440p 屏幕分享、观看提示、房间动效、玻璃材质与声音反馈。详见 [v3.0.3 更新公告](./docs/release-notes/v3.0.3.md)。

- **3.0.2**：修复进房、换座和空闲后台占用，角色位移改用合成层关键帧，语音分析与高刷新率解耦，并修复静音 Peer/过期房间会话的恢复循环；同时完善中文录音转录、整理和六套官方 ASR 选择。同版本修正版还修复了首批安装包 AI runner 清单哈希错误导致的启动失败。详见 [v3.0.2 更新公告](./docs/release-notes/v3.0.2.md)与 [稳定性参考](./docs/stabilization-reference-3.0.2.md)。
...
版本细节见 [CHANGELOG.md](./CHANGELOG.md)，普通用户说明见 [v3.0.4 更新公告](./docs/release-notes/v3.0.4.md)。
