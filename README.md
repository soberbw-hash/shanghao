...
- 第四、第五位好友加入时串行化每位成员的协商操作，只有真正解码出可播放 PCM 后才关闭该成员的语音兜底
- 共享远端音频混音器，减少多人房间的 AudioContext 数量和设备切换故障
- WebRTC 失败先对单个好友执行 ICE restart，再按需局部重建；信令断线持续退避重连
- μ-law 低带宽语音兜底按好友定向启用，带自适应语音活动门控、音频 epoch 隔离、自动 resync 与背压丢帧
...
- 固定 1440p 屏幕分享、可选系统音频、直接媒体流和独立可缩放观看窗口
- 屏幕分享由单一 Manager 管理，独立观看窗口使用最小权限 Preload
- Windows 正式包使用明确管理员权限、延迟计划任务自启与应用级 TCP/UDP 防火墙规则
- TURN 支持 UDP/TCP 与可选 TURNS，客户端只取得有时效的临时凭据
