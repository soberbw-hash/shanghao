...
- 当前屏幕分享清晰度与防火墙修复整理为 v2.9.2，构建号为 `2026.08.15.3`。
- 本地工作区仍是唯一最新基线；本次提交保留并汇总现有全部产品修改，不从远端回灌代码。
- 本轮经产品负责人明确授权：更新版本说明、生成 Windows 安装包并推送源码到 GitHub `main`。
- 本轮经明确授权创建 v2.9.2 Git Tag 和 GitHub Release；不部署信令服务器，也不改变协议版本和生产服务器版本门禁。
...
- 更新 README、CHANGELOG、版本历史和 `docs/release-notes/v2.9.2.md`。
- 创建 GitHub Release，并上传 Windows 安装包、blockmap、更新元数据和 SHA-256 校验文件。
