...
- 当前基线：ShangHao 3.0.3，完整继承 3.0.2 性能稳定改造，并加入账号、模型、录音、分享与视觉反馈修复。
- 本地工作区是唯一最新基线；不从远端覆盖本地，不重建项目。
- 用户已明确授权：全部修复和发布检查通过后，打包 Windows EXE、推送 GitHub 并发布 v3.0.3。
...
- 转录模型、转录结果和录音仍保存在本地。用户选择“房间云端”整理或问答时，相关文字会经已加入的房间连接发送到自托管 Relay；云端 API 密钥只允许存在服务器环境变量中。
