...
test("v3.0.2 release metadata and safeguards are complete", () => {
  ...
  assert.equal(rootPackage.version, "3.0.2");
  assert.equal(desktopPackage.version, "3.0.2");
  assert.equal(APP_PROTOCOL_VERSION, "7");
  assert.equal(APP_BUILD_NUMBER, "2026.08.22.1");
