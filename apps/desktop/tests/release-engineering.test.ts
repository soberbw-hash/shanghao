...
test("v2.9.2 release metadata and safeguards are complete", () => {
  ...
  assert.equal(rootPackage.version, "2.9.2");
  assert.equal(desktopPackage.version, "2.9.2");
  assert.equal(APP_PROTOCOL_VERSION, "7");
  assert.equal(APP_BUILD_NUMBER, "2026.08.15.3");
