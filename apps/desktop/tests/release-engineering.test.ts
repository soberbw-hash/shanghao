...
test("v2.1.0 release metadata and safeguards are complete", () => {
  ...
  assert.equal(APP_BUILD_NUMBER, "2026.07.26.1");
  assert.equal(existsSync(path.join(root, "docs/release-notes/v2.1.0.md")), true);
  assert.equal(changelog.includes("## 2.1.0"), true);
