...
test("v2.3.0 release metadata and safeguards are complete", () => {
  ...
  assert.equal(desktopPackage.version, "2.3.0");
  assert.equal(APP_PROTOCOL_VERSION, "6");
  assert.equal(APP_BUILD_NUMBER, "2026.08.02.1");
  assert.equal(existsSync(path.join(root, "docs/release-notes/v2.3.0.md")), true);
  assert.equal(changelog.includes("## 2.3.0"), true);
