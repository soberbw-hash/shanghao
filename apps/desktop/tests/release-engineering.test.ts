...
test("v2.9.1 release metadata and safeguards are complete", () => {
  ...
  assert.equal(rootPackage.version, "2.9.1");
  assert.equal(desktopPackage.version, "2.9.1");
  assert.equal(APP_PROTOCOL_VERSION, "7");
  assert.equal(APP_BUILD_NUMBER, "2026.08.15.2");
