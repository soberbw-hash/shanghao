...
test("v2.2.2 release metadata and safeguards are complete", () => {
  ...
  assert.equal(desktopPackage.version, "2.2.2");
  assert.equal(APP_PROTOCOL_VERSION, "6");
  assert.equal(APP_BUILD_NUMBER, "2026.08.01.1");
  assert.equal(existsSync(path.join(root, "docs/release-notes/v2.2.2.md")), true);
  assert.equal(changelog.includes("## 2.2.2"), true);
