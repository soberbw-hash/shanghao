...
test("v2.6.0 release metadata and safeguards are complete", () => {
  ...
  assert.equal(rootPackage.version, "2.6.0");
  assert.equal(desktopPackage.version, "2.6.0");
  assert.equal(APP_PROTOCOL_VERSION, "7");
  assert.equal(APP_BUILD_NUMBER, "2026.08.12.1");
