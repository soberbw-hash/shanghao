...
export const RoomHeroCard = ({
  ...
  <GlassPanel className="grid gap-5 p-5 lg:grid-cols-[1.08fr_0.92fr]">
    ...
    <div className="rounded-[18px] border border-[#E7ECF2] bg-[#F8FAFC] p-4">
      <div className="mb-1 flex items-center gap-2 text-sm font-medium text-[#111827]">
        <Link2 className="h-4 w-4 text-[#4DA3FF]" />
        加入房间
      </div>
      <div className="mb-3 text-sm text-[#667085]">粘贴房主分享的地址，然后直接加入。</div>
      <div className="space-y-3">
        <Input
          placeholder="ws://host:43821"
          value={joinSignalUrl}
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Button
            onClick={onJoinRoom}
            disabled={Boolean(isStarting || isJoining || !joinSignalUrl.trim())}
          >
            {isJoining ? "正在加入…" : "立即加入"}
          </Button>
        </div>
