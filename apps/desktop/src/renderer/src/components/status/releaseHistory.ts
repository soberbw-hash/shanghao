...
export const RELEASE_HISTORY: readonly ReleaseHistoryEntry[] = [
  {
    version: "3.0.8",
    date: "2026-09-05",
    title: "录音转录更稳，AI 和房间体验也一起收了口",
    summary:
      "这一版重点解决长录音测试中断、清除状态后无法重新选择模型，以及转录文字过碎的问题；同时把本地整理、登录进房、麦克风菜单和设置页性能整理成更直接的使用流程。",
    highlights: [
      "🎙️ 长录音更可靠：测试任务可恢复、可重试，清除本轮状态后仍可重新选择模型并从头开始。",
      "📖 转录更好读：底层精细时间轴完整保留，阅读时按说话人、停顿和长度自动合成自然段。",
      "🧠 本地 AI 更清楚：增加独立整理模型与运行时管理，下载、准备、失败原因和资源占用都有明确状态。",
      "⚡ 日常操作更利落：AI 设置页、图片查看和音频菜单减少同步重活，房间成员短暂波动不再频繁闪烁。",
    ],
    details: [
      {
        title: "录音、转录与整理",
        items: [
          "模型测试状态与普通转录隔离，完成度按真实任务范围计算，空结果、超时、崩溃和静音分别处理。",
          "原生房间录音优先继承真实用户与音轨身份，模型说话人标签不会覆盖已有成员关系。",
          "整理输入使用可读对话段落并保留原始时间引用；结果默认只保存在本地，上传由用户明确触发。",
        ],
      },
      {
        title: "房间与界面",
        items: [
          "登录后的进房确认、默认房间和未占用角色分配收口，麦克风异常不会再先闪进房间再退出。",
          "麦克风体检与说话模式移到主页音频菜单，界面音效保持默认开启，设置页面移除重复控制。",
          "移除与游戏定位无关的工作软件展示，压缩模型卡片和无效说明，新增状态不会再撑乱布局。",
        ],
      },
    ],
  },
  {
    version: "3.0.7",
    date: "2026-08-29",
    title: "这次不加花活，专门把那些膈应人的卡顿和断音揪出来",
    summary:
      "快捷音乐终于会老老实实播完整首，鼠标侧键不再时灵时不灵；设置切页、图片放大和房间音频菜单也做了针对性减负。顺手把登录配置钉进了发布验收，免得下完新包才发现账号服务没带上。",
    highlights: [
      "🎵 长音乐不再半路闭麦：原始 AAC 不裁切，播放器支持完整读取和分段续播。",
      "🖱️ 鼠标侧键更靠谱：音量调整不会再疯狂重注册 Mouse4 / Mouse5，快捷播放稳定触发。",
      "✨ 丝滑是修出来的：AI、录音库会在空闲时预热，图片预览清掉误套的全屏模糊重绘，同时保留原有动效和玻璃质感。",
      "🔐 登录配置不再靠运气：正式包会注入 CloudBase 配置，打包验收发现缺失就直接拦住。",
    ],
    details: [
      {
        title: "你能直接感觉到的变化",
        items: [
          "右侧快捷音乐与快捷键播放共用完整 AAC 流，长音频可以正常播放到结尾。",
          "设置页访问过的内容会继续留在内存里，重页面在空闲时间提前准备，切换时不再先愣一下。",
          "聊天图片打开前先隐藏未解码首帧，去掉误继承到全屏画布的昂贵滤镜，圆角、阴影、遮罩和展开动画全部保留。",
          "关于上号的“作者与项目”新增紧凑官网入口，和 GitHub 放在同一组，不另占一张卡片。",
        ],
      },
    ],
  },
  {
    version: "3.0.6",
    date: "2026-08-28",
    title: "这一版，终于把卡顿和小毛刺认真收拾了",
    summary:
      "这次不是花架子更新：页面切换、图片打开、房间进出和快捷音乐菜单都做了稳定性处理，账号与手机号登录也继续沿用当前 CloudBase 流程。",
    highlights: [
      "设置页切换更顺：常用页面提前准备，切过去不会先白一下再慢吞吞出现。",
      "图片和菜单少掉帧：大图打开、缩放、房间里的音频菜单都减少了不必要的重绘。",
      "进房状态更可靠：角色不会因为连接确认慢就走出去又退回来，等候中的状态也会一直显示。",
      "账号体验继续收口：手机号自动按 +86 处理，CloudBase 会话和登录提示更清楚。",
    ],
    details: [
      {
        title: "这次你会直接感觉到的变化",
        items: [
          "修复设置页面切换时的短暂空白和卡顿感，保留原有页面视觉与动画。",
          "修复房间快捷音乐操作触发连接断开的问题，并保留既有房间、语音和屏幕分享逻辑。",
          "发布 Windows x64 安装包，同时更新官网的版本信息和下载镜像。",
        ],
      },
    ],
  },
  {
    ...
    title: "让软件少忙一点，也更稳一点",
    summary:
      "这一版主要做收口：让后台少做重复工作，录音清理更安全，诊断用到时再加载，模型连续使用也更稳。日常的房间语音、屏幕分享和本地 AI 都保持原样。",
    highlights: [
      "后台少跑几套重复检查：诊断页面、Windows 状态等，用到时再加载，软件刚打开不会先忙一轮。",
      "录音清理更放心：自动清理先放进回收站，收藏和带标记的录音不会碰。",
      "模型接力更稳：保留现有模型，处理完会主动释放资源，不让显存一直被占着。",
      "这轮是稳定性收口，不会改变你平时进房、说话、分享屏幕的用法。",
    ],
    details: [
      {
        title: "本地审核边界",
  ...
  },
  {
    version: "3.0.4",
    date: "2026-08-26",
    title: "模型对比和快捷语音，终于更像能用的工具了",
    summary:
      "这一版主要围绕两个高频场景：拿同一段录音比较不同模型，以及在房间里快速发语音/音乐。现在可以暂停、继续、重试和导出，设置页也不再挤成一团。",
    highlights: [
      "同一段录音可以让多个模型排队跑，谁快、谁慢、谁失败，一眼就能看到。",
      "中途切页面甚至重开软件，也能接着跑；某个模型不满意，单独重试就行。",
      "快捷语音和音乐库支持搜索、分类、试听、添加反馈和导出，五个快捷键也能直接用。",
      "账号、AI 和设置页做了减法，常用入口更近，没用的信息少一点。",
    ],
    details: [
      {
        title: "模型对比与录音",
  ...
  },
  {
    ...
    details: [
      ...
      },
      {
        title: "语音与悬浮窗",
        items: [
          "好友响度：默认按约 -16 LUFS 平滑好友说话音量，并保留逐人音量和本地静音。",
